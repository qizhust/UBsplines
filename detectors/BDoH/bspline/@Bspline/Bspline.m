...
methods
    ...
    function nobj = diff(obj, ndif, dm)        
        ...
        mirrordm = zeros(size(obj.bsh));
        mirrordm(dm) = ndif;
        
        nc = mirrorpad(obj.c, mirrordm);
        nc = diff(nc,ndif,dm);
    ...
    function obj2 = basis(obj, btype, dm)
        ...
        for dm = 1:dms
            if(strcmp('dual',btype_new{dm}) && strcmp('basic',obj2.btype{dm}))
                dc = bsplineNidtrans(dc, obj2.N(dm)*2 + 1);
                obj2.btype{dm} = 'dual';
            elseif(strcmp('basic',btype_new{dm}) && strcmp('dual',obj2.btype{dm}) )
                dc = bsplineNdtrans(dc, obj2.N(dm)*2 + 1);
                obj2.btype{dm} = 'basic';
            end
            dc = shiftdim(dc, 1);
        end
