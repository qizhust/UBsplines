function [bkps,bhss] = bk_surf(img,thresh)
...
S = 3;
omin = 0;
O = 4; % Up to 32x32 images
sigma0 = 1.6*2^(1/S); % initial scale
sigman = 0; % nominal blur
verb = 1; % describe what is going on
...
bhss = bhes_ss_surf(img,sigman,O,S,omin,-1,S,sigma0);
...
% 	sigma = 2^(o-1+bhss.omin)*bhss.sigma0*2.^(oframes(3,:)/bhss.S);
    sigma = 2 * (oframes(3,:)+1) * 2^(o-1) + 3 + 5 * (2^(o-1)-1);
	bkps  = [bkps;[x(:),y(:),sigma(:)*1.2/3]];
